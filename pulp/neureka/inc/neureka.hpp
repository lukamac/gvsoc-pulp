/*
 * Copyright (C) 2020-2024  GreenWaves Technologies, ETH Zurich, University of
 * Bologna
 *
...
/*
 * Authors: Francesco Conti, University of Bologna & GreenWaves Technologies
 * (f.conti@unibo.it) Germain Haugou, GreenWaves Technologies
 * (germain.haugou@greenwaves-technologies.com) Arpan Suravi Prasad, ETH Zurich
 * (prasadar@iis.ee.ethz.ch)
 */
...
void print2DArray(const std::array<std::array<T, M>, N>& arr, int rowStart,
...
void print3DArray(const std::array<std::array<std::array<T, L>, M>, N>& arr,
                  int depthStart, int depthEnd, int rowStart, int rowEnd,
...
public:
  ...
  public:
    IoMasterDemux() {};
    IoMasterDemux(Neureka* neureka) { neureka_ = neureka; }

    inline vp::IoReqStatus req(vp::IoReq* req) {
      return neureka_->reg_config_.config0.weight_from_wmem
                 ? neureka_->wmem_port.req(req)
                 : neureka_->tcdm_port.req(req);
    }
  } demux_port;

  StateParams<std::chrono::high_resolution_clock::time_point> cpu_cycles_start,
      cpu_cycles_end;
...
private:
  ...
  std::array<ProcessingEngine<Neureka, InFeatType, SignedInFeatType,
                              OutFeatType, OutFeatType>,
             NeurekaTotalPECountXY>
      pe_instances;
  ...
  // Streamer<Neureka, StreamerDataType> normquant_streamer_instance;
  ...
  void WeightLoad(int& latency,
                  std::array<StreamerDataType, WmemBandwidthInBytes>& weight);
  void
  WeightUnpack(Mode filter_mode,
               std::array<StreamerDataType, WmemBandwidthInBytes>& weight,
               std::array<std::array<InFeatType, NeurekaBinConvPerColumnCount>,
                          NeurekaColumnPerPECount>& weight_unpacked,
               int num_bits);
  void Accumulate(
      const std::array<std::array<InFeatType, NeurekaBinConvPerColumnCount>,
                       NeurekaColumnPerPECount>& weight_array);
