...
public:
  ...
  void SetConfig(RegConfig reg_config){
    reg_config_ = reg_config;
    this->normquant_instance_.SetConfig(reg_config);
  }
  ...
  void NormQuantMult(const std::array<int,4>& index, const std::array<int,4>& enable, const std::array<NormType,4>& kappa,  const std::array<NormType,4>& shift){
    ...
    accum_scaled = this->normquant_instance_.NormQuantMult(accum, kappa, shift);
    for(int i=0; i<size; i++)
