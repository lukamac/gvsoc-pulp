...
void Neureka::StreaminSetup() {
  ...
  StreamerConfig config = this->ctrl_instance.GetStreaminStreamerConfig();
  this->streamin_streamer_instance.Init(config.base_addr, config.stride.d0, config.stride.d1, config.stride.d2, config.length.d0, config.length.d1, config.length.d2);
  this->ctrl_instance.ResetStreaminIteration();
  if(this->trace_config.setup.streamin)
    this->trace.msg("Streamin Setup is done addr : 0x%x, strides( d0 : 0x%x, d1 : 0x%x, d2 : 0x%x), lengths(d0 : %d, d1 : %d, d2 : %d)\n", config.base_addr, config.stride.d0, config.stride.d1, config.stride.d2, config.length.d0, config.length.d1, config.length.d2);
}
