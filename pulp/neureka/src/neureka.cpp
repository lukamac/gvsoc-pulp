...
Neureka::Neureka(vp::ComponentConf& config) : vp::Component(config) {
  for (int i = 0; i < NeurekaTotalPECountXY; i++)
    this->pe_instances[i] =
        ProcessingEngine<Neureka, InFeatType, SignedInFeatType, OutFeatType, OutFeatType>(this);

  this->ctrl_instance = Control<Neureka>(this);
  this->regconfig_manager_instance =
      RegConfigManager<Neureka>(this, NeurekaRegisterContextCount, NEUREKA_NB_REG);
  ...
  this->new_reg("neureka_busy", &this->busy,
                8); // Busy is high when neureka is operating
  this->busy.set(0);
  this->state.set(IDLE);

  this->new_master_port("tcdm_port", &tcdm_port);
  this->new_master_port("wmem_port", &wmem_port);
  demux_port = IoMasterDemux(this);
  this->new_master_port("irq", &irq);

  this->cfg_port.set_req_meth(&Neureka::hwpe_slave);
  this->new_slave_port("cfg_port", &cfg_port);
  ...
  this->infeat_streamer_instance = Streamer<L1BandwidthInBytes>(&tcdm_port, &io_req, &trace);
  this->streamin_streamer_instance = Streamer<L1BandwidthInBytes>(&tcdm_port, &io_req, &trace);
  this->outfeat_streamer_instance = Streamer<L1BandwidthInBytes>(&tcdm_port, &io_req, &trace);
  this->weight_streamer_instance = Streamer<WmemBandwidthInBytes>(&demux_port, &io_req, &trace);
  this->normquant_shift_streamer_instance =
      Streamer<L1BandwidthInBytes>(&tcdm_port, &io_req, &trace);
  this->normquant_bias_streamer_instance =
      Streamer<L1BandwidthInBytes>(&tcdm_port, &io_req, &trace);
  this->normquant_mult_streamer_instance =
      Streamer<L1BandwidthInBytes>(&tcdm_port, &io_req, &trace);
}

void Neureka::reset(bool active) {
  this->regconfig_manager_instance.ctxt_job_id_[0] =
      this->regconfig_manager_instance.ctxt_job_id_[1] = -1;
...
// The `hwpe_slave` member function models an access to the NEUREKA SLAVE
// interface
vp::IoReqStatus Neureka::hwpe_slave(vp::Block* __this, vp::IoReq* req) {

  Neureka* _this = (Neureka*)__this;

  if (_this->trace_level == L1_ACTIV_INOUT || _this->trace_level == L2_DEBUG ||
      _this->trace_level == L3_ALL) {
    _this->trace.msg(vp::Trace::LEVEL_DEBUG,
                     "Received request (addr: 0x%x, size: 0x%x, is_write: %d, data: %p\n",
  ...
  if (req->get_is_write()) {
    if (((addr_without_offset & 0xfff) - 0x20) >> 2 == NEUREKA_SPECIAL_TRACE_REG) {
      ...
      if (!_this->regconfig_manager_instance.job_running_ &&
          !_this->fsm_start_event->is_enqueued() && *(uint32_t*)data == 0) {
      ...
      if (addr_without_offset >= 0x20) {
        ...
        _this->regconfig_manager_instance.RegfileWrite((addr_without_offset - 0x20) >> 2,
                                                       *(uint32_t*)data);
      } else {
        // not implemented or not needed at the moment. Just kept it to make it
        // compatible with the hardware specs
        _this->trace.msg(vp::Trace::LEVEL_DEBUG,
                         "accessing special registers index: %d data: %08x\n",
    ...
    if (addr_without_offset == 0x4) {
      ...
      *(uint32_t*)data = ((_this->regconfig_manager_instance.ctxt_job_id_[0] >= 0 ? 0x1 : 0) |
                          (_this->regconfig_manager_instance.ctxt_job_id_[1] >= 0 ? 0x100 : 0));
      ...
      *(uint32_t*)data = _this->regconfig_manager_instance.RegfileRead(
          (addr_without_offset - 0x20) >> 2, _this->pending_frames);
      if (_this->trace_level == L1_ACTIV_INOUT || _this->trace_level == L2_DEBUG ||
          _this->trace_level == L3_ALL) {
        _this->trace.msg("Returning %x\n", *(uint32_t*)data);
      }
...
int Neureka::build() { return 0; }

extern "C" vp::Component* gv_new(vp::ComponentConf& config) { return new Neureka(config); }
